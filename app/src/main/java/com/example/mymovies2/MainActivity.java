...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<JSONObject> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerViewPosters.setLayoutManager(new GridLayoutManager(this, getColomnCount()));
        movieAdapter = new MovieAdapter();
        recyclerViewPosters.setAdapter(movieAdapter);
        aSwitchSort.setChecked(true);
        aSwitchSort.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
        ...
        movieAdapter.setOnReachEndListener(new MovieAdapter.OnReachEndListener() {
            @Override
            public void onReaachEnd() {
                if (!isLoading) {
                    downloadData(methodOfSort, page);
                }
            }
        });
        LiveData<List<Movie>> moviewFromLiveData = viewModel.getMovies();
        moviewFromLiveData.observe(this, new Observer<List<Movie>>() {
            @Override
